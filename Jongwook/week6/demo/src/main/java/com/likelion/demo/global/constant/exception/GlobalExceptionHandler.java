...
import com.likelion.demo.global.constant.response.code.ErrorResponse;
import com.likelion.demo.global.constant.response.code.ErrorResponseCode;
...
public class GlobalExceptionHandler {
    ...
    // @RequestBody 객체의 필드에 @Size, @NotBlank
    ...
    public ResponseEntity<ErrorResponse<?>>handleBindException(BindException e) {
        log.error("BindExceiption : {}", e.getMessage(), e);
        ErrorResponse<?> errorResponse = ErrorResponse.of(ErrorResponseCode.INVALID_HTTP_MESSAGE_BODY, e.getFieldError().getDefaultMessage());
        return ResponseEntity.status(errorResponse.getHttpStatus()).body(errorResponse);
    }
    ...
    public ResponseEntity<ErrorResponse<?>>handleHttpMessageNotReadableException(HttpMessageNotReadableException e) {
        log.error("HttpMessageReadableException : {}", e.getMessage(), e);
    ...
    // Multipart/for-data file, image
    @ExceptionHandler(MissingServletRequestPartException.class)
    public ResponseEntity<ErrorResponse<?>>handleMissingServletRequestException(MissingServletRequestPartException e) {
    ...
    // 비즈니스 로직 에러(중복 데이터 생성 요청, 존재하지 않는 회원 조회, 실제 서비스에서 발생할 수 있는 예외)
